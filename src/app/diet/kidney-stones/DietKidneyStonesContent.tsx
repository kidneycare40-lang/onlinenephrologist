...
const dietRules = [
  { icon: '💧', title: 'Drink More Water', tips: ['At least 2-3 litres per day', 'More in summer or if active'], color: 'from-blue-50 to-blue-100 border-blue-200', iconBg: 'bg-blue-100' },
  { icon: '🧂', title: 'Reduce Sodium', tips: ['Limit salt to less than 2g/day', 'Avoid packaged & fast foods'], color: 'from-amber-50 to-amber-100 border-amber-200', iconBg: 'bg-amber-100' },
  { icon: '🥩', title: 'Moderate Protein', tips: ['Don\'t eat excessive red meat', 'Balance animal and plant protein'], color: 'from-red-50 to-red-100 border-red-200', iconBg: 'bg-red-100' },
  { icon: '🥬', title: 'Eat Calcium-Rich Foods', tips: ['Include milk, curd in diet', 'Dietary calcium prevents stone formation'], color: 'from-green-50 to-green-100 border-green-200', iconBg: 'bg-green-100' },
];

const recommendedFoods = [
  { group: 'Water', options: 'Plain water, Lemon water, Coconut water', emoji: '💧' },
  { group: 'Fruits', options: 'Lemon, Orange, Watermelon, Apple', emoji: '🍋' },
  { group: 'Vegetables', options: 'Bottle Gourd, Cucumber, Leafy greens', emoji: '🥬' },
  { group: 'Dairy', options: 'Milk, Curd, Paneer (moderate)', emoji: '🥛' },
  { group: 'Grains', options: 'Rice, Roti, Oats, Dalia', emoji: '🍚' },
];

const foodsToLimit = [
  { food: 'Spinach', reason: 'High in oxalate — main cause of stones', emoji: '🥬' },
  { food: 'Tomato', reason: 'High in oxalate — use in small quantities', emoji: '🍅' },
  { food: 'Beetroot', reason: 'Very high in oxalate', emoji: '🟣' },
  { food: 'Nuts & Seeds', reason: 'High in oxalate and phosphorus', emoji: '🥜' },
  { food: 'Red Meat', reason: 'High purine — increases uric acid stones', emoji: '🥩' },
];

const foodsToAvoid = [
  { category: 'High Oxalate', examples: 'Spinach, Rhubarb, Beets, Chocolate, Nuts', emoji: '🚫' },
  { category: 'Excess Salt', examples: 'Pickles, Papad, Chips, Canned soups', emoji: '🧂' },
  { category: 'Sugary Drinks', examples: 'Cola, Energy drinks, Packaged juices', emoji: '🥤' },
  { category: 'Excess Animal Protein', examples: 'Red meat, Organ meats, Excessive chicken', emoji: '🥩' },
  { category: 'Others', examples: 'Alcohol, Excess caffeine, Vitamin C supplements', emoji: '⛔' },
];

const mealPlan = [
  { time: 'Morning', icon: '🌅', items: ['Warm lemon water', 'Oats with milk'], color: 'from-orange-100 to-amber-50' },
  { time: 'Breakfast', icon: '🍳', items: ['Whole wheat toast', 'Scrambled egg', 'Apple'], color: 'from-yellow-100 to-orange-50' },
  { time: 'Lunch', icon: '☀️', items: ['2 Chapati', 'Bottle gourd sabzi', 'Cucumber raita', 'Rice (small)'], color: 'from-blue-100 to-sky-50' },
  { time: 'Evening', icon: '🌤️', items: ['Coconut water', 'Roasted chana'], color: 'from-purple-100 to-pink-50' },
  { time: 'Dinner', icon: '🌙', items: ['2 Chapati', 'Mixed vegetable curry', 'Small piece of fish'], color: 'from-indigo-100 to-blue-50' },
];

const importantNotes = [
  'Drink plenty of water throughout the day — this is the single most important step.',
  'Get a stone analysis done to know which type of stone you have.',
  'If you have recurrent stones, get 24-hour urine testing for oxalate, calcium, and uric acid.',
  'Follow up with a urologist or nephrologist for personalized prevention.',
];

const dependsOn = [
  { label: 'Stone Type', icon: '💎' },
  { label: 'Urine pH', icon: '🧪' },
  { label: 'Oxalate Level', icon: '🔬' },
  { label: 'Calcium Level', icon: '⚗️' },
  { label: 'Uric Acid', icon: '🩸' },
  { label: 'Hydration Status', icon: '💧' },
];
...
export default function DietKidneyStonesContent() {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-white">
        <section className="relative bg-gradient-to-br from-amber-500 via-yellow-500 to-amber-700 overflow-hidden">
          <div className="absolute inset-0 opacity-10">
            <div className="absolute top-10 left-10 w-72 h-72 bg-white rounded-full blur-3xl" />
            <div className="absolute bottom-10 right-10 w-96 h-96 bg-yellow-300 rounded-full blur-3xl" />
          </div>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-20 relative">
            <nav className="text-amber-200 text-sm mb-6 flex items-center gap-2 flex-wrap">
              <Link href="/" className="hover:text-white transition-colors">Home</Link><span>›</span>
              <Link href="/diet/ckd" className="hover:text-white transition-colors">Diet Guides</Link><span>›</span>
              <span className="text-white">Diet for Kidney Stones</span>
            </nav>
            <div className="grid lg:grid-cols-2 gap-10 items-center">
              <div>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white leading-tight mb-4">
                  Diet for<br /><span className="text-amber-200">Kidney Stones</span>
                </h1>
                <p className="text-amber-100 text-base sm:text-lg mb-8 max-w-lg leading-relaxed">
                  Prevention is better than surgery. The right diet can stop kidney stones from forming and help pass small stones naturally.
                </p>
                <div className="flex flex-wrap gap-3">
                  <button onClick={handleDownloadPDF} className="inline-flex items-center gap-2 px-6 py-3 bg-white text-amber-700 font-bold rounded-xl hover:bg-amber-50 transition-colors shadow-lg cursor-pointer">
              ...
              <div className="hidden lg:flex justify-center">
                <div className="relative w-80 h-80">
                  ...
                  <div className="relative bg-gradient-to-br from-amber-400 to-yellow-500 rounded-3xl flex items-center justify-center text-8xl shadow-2xl">💎💧</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-center text-gray-900 mb-2">1. Key Diet Rules for Kidney Stones</h2>
          <div className="w-20 h-1 bg-amber-500 mx-auto rounded-full mb-10" />
            ...
            {dietRules.map((rule, i) => (
              <div key={i} className={`bg-gradient-to-b ${rule.color} border rounded-2xl p-6 text-center hover:shadow-lg transition-shadow`}>
                ...
                <ul className="text-sm text-gray-600 space-y-1.5">
                  {rule.tips.map((tip, j) => (<li key={j} className="flex items-start gap-2"><span className="text-amber-500 mt-0.5">•</span><span>{tip}</span></li>))}
                </ul>
              </div>
            ))}
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="grid lg:grid-cols-3 gap-6">
              <div className="bg-white rounded-2xl shadow-sm border border-green-200 overflow-hidden">
                ...
                <div className="px-5 py-3 bg-green-50 border-t border-green-100"><p className="text-green-700 text-xs flex items-center gap-1.5"><span>💧</span> Water is the best prevention. Aim for pale yellow urine.</p></div>
              </div>
              <div className="bg-white rounded-2xl shadow-sm border border-amber-200 overflow-hidden">
                ...
                <div className="px-5 py-3 bg-amber-50 border-t border-amber-100"><p className="text-amber-700 text-xs flex items-center gap-1.5"><span>💡</span> Cook spinach in boiling water and discard water to reduce oxalate.</p></div>
              </div>
              <div className="bg-white rounded-2xl shadow-sm border border-red-200 overflow-hidden">
                ...
                <div className="px-5 py-3 bg-red-50 border-t border-red-100"><p className="text-red-700 text-xs flex items-center gap-1.5"><span>⚠️</span> Cola and sugary drinks increase uric acid stone risk significantly.</p></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
          ...
          <p className="text-center text-gray-500 text-sm mb-10">(Kidney Stone Prevention — Example)</p>
          ...
          <div className="mt-6 bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3 max-w-3xl mx-auto">
            <span className="text-amber-600 text-lg mt-0.5">💧</span>
            <p className="text-amber-800 text-sm"><strong>Golden rule:</strong> Drink enough water to produce at least 2 litres of urine per day. Your urine should be pale yellow, not dark.</p>
          </div>
        </section>
              ...
              <div className="space-y-4">
                {importantNotes.map((note, i) => (
                  <div key={i} className="flex items-start gap-3 bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                    <div className="w-7 h-7 bg-amber-100 rounded-full flex items-center justify-center shrink-0 mt-0.5"><svg className="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div>
                    <p className="text-gray-700 text-sm leading-relaxed">{note}</p>
                  </div>
                ))}
              </div>
              <div className="bg-gradient-to-br from-amber-500 to-yellow-700 rounded-2xl p-8 text-white text-center shadow-xl">
                <div className="w-20 h-20 bg-white/15 rounded-full flex items-center justify-center text-4xl mx-auto mb-4">💎</div>
                <h3 className="text-xl font-bold mb-2">Know Your Stone Type</h3>
                <p className="text-amber-200 text-sm mb-6">Stone prevention depends on:</p>
          ...
          <div className="bg-gradient-to-r from-blue-50 via-white to-green-50 rounded-3xl border border-gray-200 shadow-lg overflow-hidden">
            <div className="grid lg:grid-cols-3 gap-0">
              ...
              <div className="p-8 flex flex-col justify-center items-center text-center border-b lg:border-b-0 lg:border-r border-gray-200">
                <h4 className="text-lg font-bold text-gray-900 mb-2">Recurrent Kidney Stones?</h4>
                <p className="text-gray-600 text-sm mb-5">Get a complete metabolic workup and personalized prevention plan.</p>
